...
describe("Send message scenario", () => {
    ...
    it('should successfully post a message', async function () {
      // posts to room 2
      const response = await request(app).post('/chat/message').query({ room_id: roomIds[1] }).set('Accept', 'application/json').send( testMessageA )
...
// todo check that faulty rooms are NOT posted by querying getRoomHistory
describe("Post new room scenario", () => {
  ...
  it('should successfully post a room when the users are peers', async function () {
    ...
    // if we make them peers, we expect the room to be posted successfully
    var badRoom = {
      name: "are they peers?", 
